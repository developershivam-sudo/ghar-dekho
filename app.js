...
// passport-local-mongoose injects the authenticate, serializeUser & deserializeUser methods to User model
passport.use(new LocalStrategy(User.authenticate()));
passport.serializeUser(User.serializeUser());
passport.deserializeUser(User.deserializeUser());

// Set flash messages to be available in all views
app.use((req, res, next) => {
    res.locals.success = req.flash("success");
    res.locals.error = req.flash("error");

    res.locals.currentUser = req.user;  // save current user info to req.locals to access into navbar.ejs
    next();
});
...
app.use("/listings/:id/reviews", reviewRouter); // :id can't be accessed in review.js
app.use("/", userRouter);
